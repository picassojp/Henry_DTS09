...
USE henry_m3;
-- Utilizar el import Wizard para importar csvs es mucho más lento, por eso se utilizan las queries

-- SELECT @@global.secure_file_priv; -- Para ver la ruta de origen donde poner los csv. No se puede cargar los archivos desde otra ruta.
...
CREATE TABLE IF NOT EXISTS `gasto` (
    ...
    `Fecha`			DATE,
  	`Monto` 		DECIMAL(10,2)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;	-- charset codificacion y collate subcodificacion, se pueden explicitar para la predeterminada

LOAD DATA INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\Gasto.csv'
INTO TABLE `gasto` 
FIELDS TERMINATED BY ',' ENCLOSED BY '' ESCAPED BY '' -- escaped by para establecer un caracter que debe ignorarse como separador
LINES TERMINATED BY '\n' IGNORE 1 -- ignore para no cargar el encabezado
...
CREATE TABLE IF NOT EXISTS `venta` (
  ...
  `IdEmpleado`			INTEGER,
  `IdProducto`			INTEGER,
  `Precio`				VARCHAR(30), -- cuando las fuentes no son muy confiables, conviene cargarlo como caracter y luego modificarlo
  `Cantidad`			VARCHAR(30)
  -- `Precio`			DECIMAL(10,2),
  -- `Cantidad`			INTEGER
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
CREATE TABLE IF NOT EXISTS sucursal (
	...
	Latitud2	VARCHAR(30), -- se carga como varchar porque tiene una coma y si se carga como decimal da error
	Longitud2	VARCHAR(30) -- se carga como varchar porque tiene una coma y si se carga como decimal da error
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
